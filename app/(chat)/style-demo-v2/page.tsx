const variants = [
  {
    ...
    bg: 'bg-[#fffaf0]',
    panel: 'bg-[#fffdfa]/82 border-[#eadfc9]',
    accent: '#002fa7',
    warm: '#f5b82e',
    heat: '#a33a2b',
    text: '#252a35',
  ...
  },
  {
    ...
    bg: 'bg-[#f8f3ea]',
    panel: 'bg-[#fffaf2]/78 border-[#dfd1bc]',
    accent: '#002fa7',
    warm: '#b8864b',
    heat: '#7f2630',
    text: '#232832',
  ...
  },
  {
    ...
    bg: 'bg-[#fffdf5]',
    panel: 'bg-white/82 border-[#ece0c8]',
    accent: '#002fa7',
    warm: '#ffd84d',
    heat: '#ff5b3f',
    text: '#22242a',
      ...
      <section className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-5 py-8 md:px-10 md:py-12">
        <header className="overflow-hidden rounded-[2.25rem] border border-[#eadfc9] bg-white/72 p-6 shadow-[0_28px_90px_-66px_rgba(55,43,31,0.55)] backdrop-blur md:p-8">
          <div className="flex flex-col gap-7 md:flex-row md:items-end md:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-base leading-8 text-[#6f6257] md:text-lg">
                这次不走普通信笺风，改成三组更有辨识度的方向：明亮、romantic、知性、苹果式简约，但不要科技感；克莱因蓝保留，加入金色或撞色，让打开页面时更开心。
              </p>
            </div>
            <div className="rounded-3xl border border-[#eadfc9] bg-[#fff8e8] p-4 text-sm leading-7 text-[#725f45] md:w-72">
              <p className="font-medium text-[#252a35]">怎么选？</p>
              <p>如果你想稳且好看，选 A。</p>
              <p>如果你想最成人，选 B。</p>
              <p>如果你想最不撞款，选 C。</p>
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          {variants.map((variant) => (
            <article
              key={variant.key}
              className={`relative flex min-h-[680px] flex-col overflow-hidden rounded-[2.25rem] border ${variant.panel} p-5 shadow-[0_24px_90px_-68px_rgba(55,43,31,0.55)] backdrop-blur`}
            >
              <div
                className="pointer-events-none absolute -right-16 -top-16 size-44 rounded-full blur-3xl"
                style={{ backgroundColor: `${variant.warm}55` }}
              />
              <div
                className="pointer-events-none absolute -bottom-20 left-8 size-44 rounded-full blur-3xl"
                style={{ backgroundColor: `${variant.accent}18` }}
              />

              <div className="relative flex items-start justify-between gap-4 border-b border-black/10 pb-5">
                <div>
                  <p className="text-xs tracking-[0.3em]" style={{ color: variant.accent }}>
                    OPTION {variant.key}
                  </p>
                  <h2 className="mt-2 text-3xl font-semibold tracking-[-0.045em]" style={{ color: variant.text }}>
                    {variant.name}
                  </h2>
              ...
              <div className="relative mt-5 flex flex-1 flex-col rounded-[1.75rem] border border-black/10 bg-white/48">
                <div className="flex items-center justify-between border-b border-black/10 px-4 py-3">
                  <div>
                    <p className="text-[11px] tracking-[0.24em]" style={{ color: variant.accent }}>
                      知己私语
                    </p>
                    <p className="mt-1 text-xs text-[#7b6b5c]">你的混乱也有语法，我慢慢读。</p>
                  </div>
                  <span
                    className="rounded-full px-3 py-1 text-xs text-white shadow-[0_16px_32px_-22px_rgba(0,47,167,0.8)]"
                ...
                <div className="flex flex-1 flex-col gap-5 px-4 py-5">
                  <div>
                    <div className="mb-2 flex items-center gap-2 text-[11px] tracking-[0.22em]" style={{ color: variant.accent }}>
                      <span className="size-1.5 rounded-full" style={{ backgroundColor: variant.accent }} />
                      TA
                    </div>
                    <div className="border-l-2 bg-white/55 py-1 pl-4 pr-2" style={{ borderColor: variant.accent }}>
                      <p className="text-sm leading-7 text-[#30323a]">
                        我在。先不用解释得很漂亮，想到哪里就说到哪里。你说不清的部分，我也会陪你慢慢看。
                      </p>
                    </div>
                  </div>

                  <div
                    className="ml-auto max-w-[84%] rounded-[1.35rem] rounded-br-sm px-4 py-3 text-sm leading-7 shadow-sm"
                    style={{ backgroundColor: `${variant.warm}32`, color: variant.text }}
                  >
                  ...
                  <div>
                    ...
                    <div className="border-l-2 bg-white/55 py-1 pl-4 pr-2" style={{ borderColor: variant.accent }}>
                      <p className="text-sm leading-7 text-[#30323a]">
                        那我们就把亲密做得聪明一点：不催促、不表演，只在你需要的时候，给你一盏灯和一句准确的话。
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4">
                  <div className="rounded-[1.45rem] border border-black/10 bg-white/70 p-3">
                    <p className="min-h-16 text-sm leading-7 text-[#9a8c7d]">把心里的话放在这里……</p>
                    <div className="mt-2 flex items-center justify-between border-t border-black/10 pt-3">
                      <span className="text-[11px] text-[#8a7a6b]">adult romance · smart softness</span>
                      <span className="flex size-8 items-center justify-center rounded-full text-sm text-white" style={{ backgroundColor: variant.accent }}>
                        ↑
                      </span>
                    </div>
                  </div>
              ...
              <div className="relative mt-5 rounded-[1.5rem] border border-black/10 bg-white/45 p-4">
                <p className="text-xs tracking-[0.25em]" style={{ color: variant.heat }}>
                  {variant.chip}
                </p>
        ...
        <section className="rounded-[2.25rem] border border-[#eadfc9] bg-white/72 p-5 shadow-[0_24px_90px_-70px_rgba(55,43,31,0.45)] backdrop-blur md:p-7">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <p className="max-w-2xl text-sm leading-7 text-[#6f6257]">
              A 的蓝金比例最适合长期使用：打开时明亮开心，但又有克莱因蓝的智性锚点；金色增加 romantic 和生活感，少量酒红负责成人恋爱的温度。
            </p>
          </div>
          ...
          <div className="mt-6 overflow-hidden rounded-[1.5rem] border border-[#eadfc9]">
            ...
            {comparison.map((row) => (
              <div key={row[0]} className="grid grid-cols-4 border-t border-[#eadfc9] bg-white/45 text-sm text-[#6f6257]">
                {row.map((cell) => (
                  <div key={cell} className="p-3 leading-6">
                    {cell}
                  </div>
                ))}
              </div>
